...
import { PQBrowser, TrendSearch } from '../../../global';
...
const TrendPlot = (props: IContainerProps) => {
    ...
    function GetEventData(timeFilter: PQBrowser.IReportTimeFilter, meters: number[], assets: number[]): JQuery.jqXHR<any[]> {
    ...
    function navigateEvent(marker: TrendSearch.IEventMarker): () => void {
        ...
        const timeFilter: PQBrowser.IReportTimeFilter = {
    ...
    const closeButton = (
        <Button key="remove-plot" onClick={() => {
    ...
    const overlayButton = widgetDefinition.Settings == null ? null : (
        <Button key="plot-settings" onClick={() => setShowSettings(!showSettings)}>
            {Pencil}
        </Button>);
    ...
    if (props.DragMode)
        ...
        plotBody = (
            <Widget
                ...
                Controls={[...markerControls, overlayButton, closeButton]}
                Overlays={overlays}
            />
        );
    }
